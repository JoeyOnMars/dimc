...
5. 自 `2026-03-19` 起，本文是当前仓库**唯一正式有效**的产品级项目架构入口。
...
9. **Projection Without Erasure**：系统允许多提取层、多置信度层或多来源结果并存；读侧可以形成“当前最佳视图”，但写侧不得用覆盖式替换抹平 provenance、来源层与历史状态。
10. **Causality Is Not Similarity**：语义相似、时间邻近、类型相同或共现只能作为候选信号，不能单独被当成因果成立依据。
11. **Relation Writes Must Be Validated**：任何进入对象关系网络的结构化关系，都必须经过统一本体与合法性校验，并保留可追溯来源；系统不得幻构无法回溯的因果边。
...
5. 关系状态与等级的知识化表示；
6. 为解释层提供可查询对象图。
7. 多提取层或多置信度层的对象结果可以并存，但必须保留来源层、来源 run 与 provenance。
...
5. Layer 2 可以产生“当前最佳投影”，但不得通过覆盖写入抹平低层结果、旧结果或来源差异。
...
4. 相似度、时间邻近和类型共现只能作为候选发现信号，不能单独构成被确认的因果关系。
5. 任何自动写入的结构化关系都必须经过本体合法性校验，并保留其声明来源、推导来源或验证来源。
