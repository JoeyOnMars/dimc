...
> **已抽升到第一层的长期原则**：
> 1. 系统必须保留 deterministic precision retrieval，而不只依赖语义召回。
> 2. 精确召回通道负责低幻觉、可验证的命中，不负责智能推理。
> 3. `search / trace / why / audit` 等上层能力都可以依赖这条精确召回底座。
>
> **本文剩余内容的层级**：以下正文继续记录 UNIX 检索通道的第二层实现契约、模块形态和阶段性实现选择，不构成第一层产品定义。
