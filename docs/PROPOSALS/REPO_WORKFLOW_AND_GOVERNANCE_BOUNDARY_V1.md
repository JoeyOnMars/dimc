...
本文用于说明当前 `dimc` 仓库中的“项目开发流程与治理边界”到底指什么，以及它在五层结构中的位置。

当前 `dimc` 共享仓库至少同时承载四层共享内容：
...
此外，本项目还真实存在第五层“本地开发控制层”，但它默认保留在本地开发目录，不构成共享仓库入口。

本文只解释第四层“仓库治理层”的边界、作用范围与维护原则。
...
6. 不把 repo-specific 协作规则写成产品能力或项目落地层本体

## 2. 五层结构中的治理位置

### 2.1 产品定义层回答什么问题

产品定义层回答的是：“DIMCAUSE 这个产品本体是什么，它解决什么问题，它依靠什么长期稳定语义运行。”
...
产品实现层回答的是：“这个产品目前被实现成什么样，代码、测试和运行现实做到哪一步。”
...
这一层属于产品本身，但不能反向重写产品定义层。

### 2.3 项目落地层回答什么问题

项目落地层回答的是：“当前仓库怎样把产品组织成一个默认可运行的本地项目。”

它负责说明：

1. 当前默认入口
2. 当前默认目录组织和打包方式
3. 当前默认 artifact / storage / index 组合如何在本项目中落地
4. 哪些内容只是本项目默认选择，而不是产品普遍要求

项目落地层可以描述当前默认选择，但不能吸收仓库治理规则。

### 2.4 仓库治理层回答什么问题

仓库治理层回答的是：“当前 `dimc` 仓库如何协作、保护文档、维护质量、控制变更、组织交付。”

它关注的是当前仓库这一协作对象的治理，不是产品运行时语义，也不是项目落地层的实例化逻辑。

换句话说：

1. 产品定义层定义“产品是什么”
2. 产品实现层承载“产品目前实现成了什么”
3. 项目落地层定义“本项目怎样承载并默认运行这个产品”
4. 仓库治理层定义“当前仓库怎样被协作、被约束、被维护”

### 2.5 本地开发控制层回答什么问题

本地开发控制层回答的是：“为了让当前开发现场继续运转，本地目录里必须保留什么。”
...
这一层与仓库治理层关系密切，但两者不是同一层：前者是共享规则，后者是本地现场控制。
...
因此，repo workflow / governance 本质上是一套“当前仓库治理机制”，而不是产品定义、产品实现或项目落地的替代层。
...
以下规则会影响“如何工作”和“如何写文档”，但它们不应被误当成产品定义或项目落地定义：
...
以下内容应明确保留在“当前 repo 内部 workflow / governance”层，而不是回流进入产品定义、产品实现或项目落地层。
...
## 6. 明确不应进入产品定义、产品实现或项目落地层的内容

以下内容如果被写入产品定义、产品实现或项目落地层，本质上都属于层级污染。
...
3. 项目落地层只能解释默认映射，不能吸收 repo-specific 治理机制。
...
1. 保留其中仍有效的仓库治理约束
2. 显式降级其中过时的产品叙事
3. 不为了表面一致而把旧叙事重新写回正式架构或项目落地层
4. 如需更新旧规则，应以“纠正分层”而不是“回收产品定义”为目标

## 8. 后续维护建议

为了避免五层结构再次混淆，后续维护建议如下：
...
1. 新增或修改规则文件时，先标明它属于哪一层：
   ...
   - 仓库治理层
   - 跨层行为约束层
2. 凡是 branch、preflight、worktree、lint gate、venv、checkpoint、审计模板之类内容，默认先归入仓库治理层，而不是产品层。
3. 凡是对象模型、证据政策、因果等级、四层职责、运行时主链之类内容，默认先归入产品定义层，而不是 rules。
4. 凡是当前本地默认材料表面、默认 artifact layout、默认 storage/index 组合之类内容，默认先归入项目落地层，而不是产品定义层。
5. 含历史叙事的规则文件应逐步补充边界说明，避免后来读者把旧语境误当作当前产品定义。
6. 当仓库治理层与项目落地层同时变化时，应优先确认“变的是默认映射，还是变的是内部治理”，避免混写。
7. 后续如需把本文从 proposal 提升为正式仓库治理说明，应保持它仍然只解释当前仓库治理边界，而不接管产品定义、产品实现或项目落地层的职责。

## 9. 结论

当前 `dimc` 仓库中的五层结构必须继续严格切开：

1. 产品定义层负责定义产品本体
2. 产品实现层负责承载产品当前实现
3. 项目落地层负责定义本项目的默认承载方式
4. 仓库治理层负责定义当前仓库如何被协作、被保护、被维护
5. 本地开发控制层负责维持当前开发现场的连续性

本 proposal 的作用，不是增加新的产品设计，而是防止当前仓库的治理现实继续污染产品定义、产品实现和项目落地。

只要这条边界不被重新混淆，旧规则可以继续作为仓库治理材料存在，正式架构、产品实现和项目落地也才能保持稳定、可维护、可迁移。
