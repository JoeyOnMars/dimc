...
- **现状**: `legacy_debt` 隔离标记已清零；当前主战场是剩余 `22 skipped + 4 deselected protected` 的测试债分批收口。

### P1-9: 全量 skipped / protected 测试债清理（当前 22 skipped + 4 deselected）
- **审计事实**: `source .venv/bin/activate && pytest -q -rs` 当前为 `1113 passed, 22 skipped, 4 deselected`。
- **分类结果（skipped=22）**:
  1. 过时测试：`0`（已清零）
  2. 手工 / 完整环境测试：`2`
  3. 环境依赖缺失：`0`（已清零）
...
- **本轮进展**:
  ...
  - 默认套件基线已从 `1103/30` 收敛到 `1113 passed, 22 skipped, 4 deselected`
