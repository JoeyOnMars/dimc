...
### P1-9: 全量 skipped / protected 测试债清理（当前 23 skipped + 4 deselected）
- **审计事实**: `source .venv/bin/activate && pytest -q -rs` 当前为 `1112 passed, 23 skipped, 4 deselected`。
- **分类结果（skipped=23）**:
  1. 过时测试：`0`（已清零）
  2. 手工 / 完整环境测试：`2`
  3. 环境依赖缺失：`1`
...
- **本轮进展**:
  ...
  - 默认套件基线已从 `1103/30` 收敛到 `1112 passed, 23 skipped, 4 deselected`
